...
export const nextPow2 = function(x): number {
  return Math.pow(2, Math.round(Math.max(x,0)).toString(2).length);
}

export const logTwo = function(n): number {
  if (Math.log2) return Math.log2(n);
...
export var calcVPMatrix = function(width, height) {
  var vMatrix, pMatrix;
  var wh = width * 0.5,
      hh = height * 0.5;
...
// export default phina.define("phina.gl2d.Util", {
//   _static: {
